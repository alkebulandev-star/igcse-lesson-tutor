...
(function () {
  ...
  // Fallback: native browser TTS (used when ElevenLabs is unreachable, no API
  // key configured, quota hit, autoplay blocked etc.). Saved to a window var
  // by the bottom of this file BEFORE we override speechSynthesis.
  function nativeFallbackSpeak(text, onend){
    ...
    try { nat.cancel(); } catch(e){}
    var u = new Ut(text);
    u.volume = 1; u.rate = 0.97; u.pitch = 1;
    // Prefer en-GB voices for the British accent
    try {
      var voices = nat.getVoices() || [];
      var pick = voices.find(function(v){ return v.lang === 'en-GB' && /serena|kate|fiona|emily|karen|martha|amy|alice|premium|enhanced|neural/i.test(v.name); })
    ...
    try { nat.speak(u); } catch(e){
      if (typeof onend === 'function') try { onend({ error:String(e) }); } catch(_){}
    }
  }
  ...
  async function play(text, onend) {
    ...
    try {
      ...
      console.warn('[ElevenLabs] TTS failed, using native browser voice:', e && e.message || e);
      diag('FALLBACK: native browser voice', '#f59e0b');
      // ── Universal fallback so every caller (kids zone, music studio, kSpeakText,
      //    lesson narration, etc.) still gets audio when the Vercel proxy is
      //    unreachable, the API key is missing, or quota is hit.
      nativeFallbackSpeak(text, onend);
    }
  }
